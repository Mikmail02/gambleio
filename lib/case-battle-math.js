...
function resolveBattleResult(participants, totalPot, mode, opts = {}) {
  ...
  const winnerParticipants = participants.filter((p) => bestTeams.includes(p.teamIndex));
  const winnerCount = winnerParticipants.length;
  const amountPerWinner = winnerCount > 0 ? totalPot / winnerCount : 0;
  payouts.forEach((p) => {
